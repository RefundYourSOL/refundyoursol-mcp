...
async function main() {
  ...
  if (hasSigningCapability()) {
    ...
    server.tool(
      'trade_token',
      'Buy or sell tokens on Solana. Supports 12+ DEXes (PumpSwap, Raydium, Meteora, Orca, etc.) with automatic routing. Fast mode (~150-400ms) or stable mode (full confirmation). Optional Jito MEV protection.',
      tradeTokenSchema.shape,
      async (input) => ({
